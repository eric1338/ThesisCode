...
namespace SongVisualizationApp.SongAnalyzing.OnSetDetection
{
	class AudioAnalysis2
	{
		...
		{
			heldNoteDetection = new HeldNoteDetection(PCMStream, 1024);
			...
			do
			{
				...
				finished = heldNoteDetection.AnalyzeSpectrum(ReadMonoPCM());
			}
			while (!finished);
		...
		{
			return heldNoteDetection.GetTimePerSample();
		}
